...
// Creating a boolean expression to return person with the longest length
...
int simple_init(void)
{
  ...
  struct birthday *longestfemale = head;
    _Bool check;
    list_for_each_entry(ptr, &birthday_list, list){
        check = compare_birthday(ptr, longestfemale);
        if (check && ptr->gender == "F" || ptr->gender == "Female")
            longestfemale = ptr;
    }

    list_del(&longestfemale->list);
    kfree(longestfemale);
